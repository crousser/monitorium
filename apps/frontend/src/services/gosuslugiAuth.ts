...
export const gosuslugiAuth = {
    ...
    initiateLogin: (forRepresentative: boolean = false) => {
        ...
        localStorage.setItem(
            'gosuslugi_auth_state',
            forRepresentative ? 'representative' : 'voter',
        );

        // Simulate redirect for the demo
        window.location.href =
            '/gosuslugi/callback?code=mock-auth-code&state=' +
            (forRepresentative ? 'representative' : 'voter');
    },
    ...
    mapUserDataToAppFormat: (
        data: GosuslugiUserData,
