    ...
    const processCallback = async () => {
        try {
            ...
            if (!code) {
                ...
                setError(
                    'Отсутствует код авторизации. Пожалуйста, попробуйте войти снова.',
                );
                setIsLoading(false);
                return;
            }
            ...
            if (!state) {
                setError(
                    'Отсутствуют данные о типе пользователя. Пожалуйста, попробуйте снова.',
                );
                setIsLoading(false);
                return;
            }
            ...
            // Get user info
            const userData = await gosuslugiAuth.getUserInfo(
                token.access_token,
                isRepresentative,
            );

            // Map to app format
            const appUser = gosuslugiAuth.mapUserDataToAppFormat(
                userData,
                isRepresentative,
            );
            ...
            navigate(
                isRepresentative ? '/representative/dashboard' : '/dashboard',
            );
        } catch (error) {
            ...
            <div className="honor-container py-12">
                <div className="max-w-md mx-auto text-center">
                    ...
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center">
                            <p>
                                Авторизация выполнена успешно. Перенаправляем
                                вас...
                            </p>
                        </div>
                    )}
